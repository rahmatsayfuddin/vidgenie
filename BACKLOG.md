...
- [x] **FEAT-015** — GIF selalu diperlakukan sebagai video pendek (animasi hidup di MP4) | **selesai**
  - Evidence:
    ...
    - Tests (7 PASS baru = 146 total): `tests/test_storage.py` (`detect_media_type("gambar.gif") == "video"`), `tests/test_media.py` (+3 — probe GIF animasi: `(64,48)` + durasi ≈ `n_frames×100 ms`; GIF frame tunggal → durasi `None`; thumbnail GIF 320×320 dari frame pertama), `tests/test_render.py` (+2 — `_segment_cmd` utk GIF memuat `-stream_loop -1` + `-i …gif` + `trim=duration=4` + `fps=30`; e2e `render_scenes` GIF nyata 2 s → job `ok` + MP4 >0 byte, `save_media("a.gif", "image/gif")` → `asset.type == "video"`), `tests/test_upload.py` (+1 — `POST /upload` GIF → 303, `type=video`, `mime=image/gif`, `width/height=64/48`, `duration_sec≈0.3`, thumb ada).
    - Verifikasi (.venv Python 3.12.6, ruff 0.16.9 / mypy 2.3.1 / pytest 9.1.1): `ruff check .` = All checks passed; `ruff format .` = 2 files reformatted; `mypy src` = Success, no issues found in 15 source files; `pytest -q` = **145 passed, 1 failed** (pra-eksisting `test_probe_video`, ffprobe SIGABRT).
    - Live (uvicorn :8000): upload `anim.gif` (5 frame, 100 ms) → `GET /assets/{id}` — Tipe `video`, MIME `image/gif`, Durasi `0.5 s`, Dimensi `160 x 280`, preview via `<img>`; `POST /plan` (3 kalimat) → compose → `POST /plan/{id}/render` → job `done` → MP4 valid (`ffmpeg -i` decode 342 frame, `time=00:00:11.40` = 3×4 s − 2×0.3 s xfade → GIF di-loop mengisi penuh durasi scene).
  - Catatan migrasi: aset GIF lama yang sudah terlanjur `type=image` tidak diproses ulang otomatis (library dev tidak berisi GIF).
